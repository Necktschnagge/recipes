...
curl -H "Authorization: token ${GH_REPO_TOKEN}" -X POST -d "{\"body\": \"See build preview here: [${git_hash}.pdf](https://github.com/Necktschnagge/recipes/blob/artifacts/artifacts/${git_hash}.pdf)\"}" "https://api.github.com/repos/${TRAVIS_REPO_SLUG}/issues/${TRAVIS_PULL_REQUEST}/comments"
else 
